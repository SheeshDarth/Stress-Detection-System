...
class TestFeatureNames:
    ...
    def test_contains_lip_depression(self):
        assert "lip_depression_mean" in FEATURE_NAMES

    def test_contains_jaw_clenching(self):
        assert "jaw_clenching_std" in FEATURE_NAMES


class TestFeatureVector:
    """Test feature vector creation."""

    def test_creates_11d_vector(self):
        visual = {
            ...
            "lip_depression_mean": 0.08, "jaw_clenching_std": 0.04,
        }
...
class TestTraining:
    ...
    def test_feature_importance_populated(self, trained_classifier):
        assert len(trained_classifier.feature_importance) == 11
...
class TestPrediction:
    ...
    def test_predicts_normal(self, trained_classifier):
        vec = np.array([15, 0.30, 0.02, 0.35, 0.01, 0.001, 0.005, 72, 45, 0.08, 0.04])
    ...
    def test_predicts_stressed(self, trained_classifier):
        vec = np.array([28, 0.22, 0.05, 0.25, 0.04, 0.006, 0.018, 100, 20, 0.25, 0.10])
        label, conf = trained_classifier.predict(vec)
        assert label in ("Normal", "Stressed")
        assert 0.0 <= conf <= 1.0

    def test_handles_nan(self, trained_classifier):
        vec = np.array([np.nan, 0.30, 0.02, 0.35, np.nan, 0.001, 0.005, 72, 45, 0.08, 0.04])
        label, conf = trained_classifier.predict(vec)
